...
-- Function to calculate earnings when a stream ends
...
BEGIN
  ...
  -- If no record exists, default to enabled; if explicitly disabled, exit
  ...
  IF _global_config IS NULL THEN
    RETURN NEW; -- No monetization configured
  END IF;
  ...
  -- Count VALID leads: only viewers who stayed until end (2-min tolerance)
  SELECT count(DISTINCT vp.stream_lead_id) INTO _valid_leads
  FROM public.live_viewer_presence vp
  WHERE vp.room_id = NEW.id
    AND vp.stream_lead_id IS NOT NULL
    AND (
      vp.left_at IS NULL
  ...
  LEFT JOIN LATERAL (
    SELECT
      ...
      count(DISTINCT vp.stream_lead_id) FILTER (
        WHERE vp.stream_lead_id IS NOT NULL
  ...
  _month_streams := _month_streams + 1; -- include current stream
...
ADD CONSTRAINT uq_streamer_earnings_host_period UNIQUE (host_id, period_start);

-- Create trigger on live_rooms
CREATE TRIGGER trg_calculate_stream_earnings
