...
-- ---------------------------------------------------------------------------
-- Parte B: restringir cursos por tier (mismo patrón que live_rooms/portal_events)
-- ---------------------------------------------------------------------------
ALTER TABLE public.academy_courses
  ADD COLUMN IF NOT EXISTS required_tiers TEXT[];   -- NULL/[] = todos; ['vip','platino'] = solo esos

-- ---------------------------------------------------------------------------
-- Parte C: membresías como producto vendible.
-- Un portal_product con product_type='membership' sube al usuario al tier
-- indicado en membership_tier cuando se paga (reference_id es UUID, no sirve
-- para un nombre de tier, por eso una columna dedicada).
-- ---------------------------------------------------------------------------
ALTER TABLE public.portal_products
  ADD COLUMN IF NOT EXISTS membership_tier TEXT;

-- ---------------------------------------------------------------------------
-- Parte D: paquetes (bundles) de varios cursos.
-- Se venden como portal_product con product_type='bundle' y reference_id = bundle.id.
-- Al pagar se inscribe al usuario en TODOS los cursos del bundle.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.academy_bundles (
  ...
  product_id     UUID,                              -- portal_product asociado (para venta)
...
-- Anon (usuarios del portal) lee bundles publicados.
CREATE POLICY "academy_bundles: anon read published"
  ON public.academy_bundles FOR SELECT TO anon
  USING (status = 'published');

-- IB admin gestiona los bundles de su portal (puente profiles → ib_id).
...
-- Anon lee la relación bundle↔curso (para mostrar qué incluye un bundle).
CREATE POLICY "academy_bundle_courses: anon read"
  ON public.academy_bundle_courses FOR SELECT TO anon
  USING (true);

-- IB admin gestiona la relación de sus bundles.
