...
-- ----------------------------------------------------------------------------
-- Reemplaza los DOS sistemas que coexistían (revenue-split en finalizeOrder +
-- MLM async vía mlm-engine) por UN ÚNICO motor atómico:
--   distribute_order_commissions(order_id), disparado por trigger SÍNCRONO al
--   pasar la orden a 'paid'. Modelo (docs/comisiones-modelo.md):
--
--     G (venta) = Bullfy Platform Fee + Pool de red (con compresión) + Socios (incl. IB)
--
--   · Platform fee: % por portal (global admin), debitado DEL 100%.
--   · Pool: niveles = % de la venta, por la UPLINE COMPRIMIDA (solo ancestros
--     activos con mlm_enabled). Lo no reclamado por la red engrosa el pot de socios.
--   · Socios (incluye al IB): reparten el pot (net − networkPaid) PRO-RATA por su %.
--     Si no hay socios, todo va al IB (host).
--   · Disponibilidad: cripto (usdt) → disponible al instante; Stripe (fiat) →
--     pendiente (hasta liquidación, Fase 5); demo → disponible.
--
-- Saldo SEGREGADO POR MÉTODO en portal_wallet_balances (usdt | stripe). DUAL-WRITE
-- al agregado legacy de portal_user_wallets para que los RPC de retiro existentes
-- sigan funcionando hasta la Fase 4.
--
-- Auditoría en TABLA PROPIA portal_commission_lines (NO reutiliza
-- portal_mlm_commissions/portal_commissions para no colisionar con crons viejos).
-- Se DESACTIVAN los caminos viejos de acreditación (crons + función portal_owner)
-- y los triggers async del mlm-engine. Idempotente. No destruye datos.
-- ============================================================================

-- ─────────────────────────────────────────────────────────────────────────
-- 1) CONFIG: Platform fee por portal + activación MLM por usuario
-- ─────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────
-- 2b) AUDITORÍA: una línea por beneficiario por orden (tabla propia)
-- ─────────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.portal_commission_lines (
  ...
  beneficiary_user_id uuid REFERENCES public.partner_users(id) ON DELETE SET NULL,  -- NULL = plataforma (Bullfy)
  beneficiary_type    text NOT NULL CHECK (beneficiary_type IN ('platform','network','socio','ib')),
  source_user_id      uuid,            -- comprador
  level_number        int,             -- solo 'network'
...
-- ─────────────────────────────────────────────────────────────────────────
-- 3) MIGRACIÓN DE DATOS: saldo real actual → bucket USDT (lo único cobrado fue
--    NOWPayments/cripto). Idempotente.
-- ─────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────
-- 4) HELPER: acreditar a un beneficiario en su bucket + dual-write + auditoría
-- ─────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.credit_wallet_method(
  ...
  _method         text,
  _amount         numeric,
  _availability   text,           -- 'available' | 'pending'
  _reference_id   uuid,
...
DECLARE
  ...
  _pool_amt     numeric(12,2);   -- tope monetario del pool de red
  _net_cap      numeric(12,2);   -- tope para networkPaid = LEAST(pool, net)
  ...
  _amt          numeric(12,2);
  _socio        RECORD;
  _sum_w        numeric(12,4) := 0;   -- suma de % de socios OTROS (excl. host)
  _total_w      numeric(12,4) := 0;   -- suma total de pesos (otros + host)
...
BEGIN
  ...
  _cur    := CASE WHEN _method = 'stripe' THEN 'USD' ELSE 'USDT' END;
  _host   := public.get_portal_host_user_id(o.portal_id);

  -- Sin host (IB aún no provisionado) NO se reparte NADA: se difiere para que el
  -- barrido de reconciliación lo reintente cuando exista el host. Así la guarda de
  -- idempotencia no se "quema" y no se pierde el residual del IB.
  IF _host IS NULL THEN RETURN; END IF;

  -- (a) Platform fee (Bullfy) — debitado del 100%. Línea de auditoría, sin wallet.
  ...
  IF COALESCE(_mlm_enabled,false) AND COALESCE(_pool_pct,0) > 0 THEN
    -- Tope monetario: la red NUNCA reparte más que el pool ni más que el net
    -- (protección anti dinero-creado ante config inválida Σniveles>pool).
    ...
    IF _eligible IS NOT NULL AND array_length(_eligible,1) > 0 THEN
      ...
      LOOP
        -- Compresión: el nivel L paga al L-ésimo ancestro ELEGIBLE (ya comprimido).
        _benef := _eligible[_lvl.level_number];
        CONTINUE WHEN _benef IS NULL;
        _amt := round(o.total_usd * _lvl.percentage / 100.0, 2);
        -- Cap defensivo: no exceder el pool ni el net restante.
  ...
  -- (c) Socios (incluye al IB): reparto del pot. Los socios OTROS (no-host) reciben
  --     pro-rata por su peso; el IB (host) recibe el RESIDUAL exacto (absorbe el
  --     pool no reclamado y el redondeo) → la suma cuadra a 100% siempre.
  _socios_pot := GREATEST(_net - _network_paid, 0);

  IF _socios_pot > 0 AND _host IS NOT NULL THEN
    -- Suma de % de socios activos EXCLUYENDO al host.
    ...
    -- Peso del host: su % configurado, o residual sintetizado si no figura.
    ...
    -- Otros socios pro-rata por el peso TOTAL.
    IF _total_w > 0 THEN
      FOR _socio IN
    ...
    -- IB (host): residual exacto del pot.
...
-- ─────────────────────────────────────────────────────────────────────────
-- 6) TRIGGER ÚNICO síncrono; se retiran los triggers del mlm-engine async.
-- ─────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────
-- 7) DESACTIVAR caminos viejos de acreditación (evitan doble conteo).
--    El nuevo motor acredita directo; ya no hay pending→available por cron para
--    cripto (es inmediato) y el fiat se liberará por el listener de Stripe (Fase 5).
-- ─────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.release_due_portal_owner_commissions()
RETURNS integer LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  SELECT 0;  -- no-op: reemplazado por distribute_order_commissions (Fase 1)
$$;

-- Barrido de reconciliación: reparte órdenes 'paid' que aún no tienen líneas
-- (host provisionado tarde, o trigger perdido). Idempotente (la función lo es).
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    -- Retirar los crons viejos de liberación (sus nombres conocidos).
    ...
    END IF;
    -- Agendar la reconciliación cada hora (red de seguridad del motor síncrono).
