...
-- Antes: la comisión 'portal_owner' solo se acreditaba al wallet (a DISPONIBLE) tras la
-- ventana de reembolso → el IB veía $0 justo tras una venta.
-- Ahora: se acredita a PENDIENTE al generarse (visible de inmediato) y se mueve a
-- DISPONIBLE al vencer la ventana — igual que las comisiones MLM.
--
-- No toca finalizeOrder ni Edge Functions (lo hace el mismo cron, ahora cada 15 min).
-- Idempotente vía pending_credited_at. Incluye BACKFILL de lo ya existente al final.

ALTER TABLE public.portal_commissions
  ADD COLUMN IF NOT EXISTS pending_credited_at timestamptz;

-- Redefine la función de la Fase 1: PASO 1 acredita a PENDIENTE lo no acreditado;
-- PASO 2 libera PENDIENTE→DISPONIBLE lo vencido. Reemplaza el modelo anterior
-- (que acreditaba directo a DISPONIBLE). Consistente con lo ya liberado (status='available').
...
DECLARE
  ...
  _released      int := 0;
BEGIN
  -- PASO 1 — acreditar a PENDIENTE las comisiones portal_owner aún no acreditadas.
  FOR _c IN
  ...
  LOOP
    ...
    IF _host IS NULL THEN CONTINUE; END IF;  -- dueño sin registro host aún → se reintenta
  ...
  -- PASO 2 — liberar PENDIENTE→DISPONIBLE las ya acreditadas y vencidas.
  FOR _c IN
...
-- Reprogramar el cron a cada 15 min (para que el PENDIENTE aparezca rápido tras la venta).
DO $$
BEGIN
...
-- BACKFILL inmediato: acredita a PENDIENTE lo ya existente para que se vea de inmediato.
SELECT public.release_due_portal_owner_commissions();
