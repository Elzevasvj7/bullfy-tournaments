
-- 1) Update calculate_stream_earnings: lead is valid only if present from start (±2 min) to end (±2 min)
CREATE OR REPLACE FUNCTION public.calculate_stream_earnings()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
DECLARE
  _mon_enabled boolean;
  _global_config RECORD;
  _custom RECORD;
  _rate_lead numeric;
  _rate_bono_streams numeric;
  _rate_bono_views numeric;
  _rate_bono_interactions numeric;
  _rate_bono_votacion numeric;
  _umbral_streams int;
  _umbral_views int;
  _umbral_interactions int;
  _umbral_votacion numeric;
  _started_at timestamptz;
  _ended_at timestamptz;
  _valid_leads int;
  _stream_viewers int;
  _stream_interactions int;
  _stream_avg_rating numeric;
  _period_start date;
  _period_end date;
  _month_streams int;
  _month_viewers int;
  _month_leads int;
  _month_interactions int;
  _month_avg_rating numeric;
  _earnings_leads numeric;
  _earnings_bonuses numeric;
  _bonus_details jsonb;
  _is_ib_externo boolean;
  _tolerance interval := interval '2 minutes';
BEGIN
  IF NEW.status <> 'ended' OR OLD.status = 'ended' THEN
    RETURN NEW;
  END IF;

  SELECT EXISTS (
    SELECT 1 FROM public.user_roles WHERE user_id = NEW.host_id AND role = 'ib_externo'
  ) INTO _is_ib_externo;

  IF NOT _is_ib_externo THEN
    RETURN NEW;
  END IF;

  _started_at := COALESCE(NEW.started_at, NEW.created_at);
  _ended_at := COALESCE(NEW.ended_at, now());

  SELECT enabled INTO _mon_enabled
  FROM public.live_streamer_monetization
  WHERE host_id = NEW.host_id;

  IF _mon_enabled IS NOT NULL AND _mon_enabled = false THEN
    RETURN NEW;
  END IF;

  SELECT * INTO _global_config FROM public.live_monetization_config WHERE active = true LIMIT 1;
  IF _global_config IS NULL THEN
    RETURN NEW;
  END IF;

  SELECT * INTO _custom FROM public.live_streamer_monetization WHERE host_id = NEW.host_id;

  _rate_lead := COALESCE(_custom.custom_dolares_por_lead, _global_config.dolares_por_lead);
  _rate_bono_streams := COALESCE(_custom.custom_bono_streams_monto, _global_config.bono_streams_monto);
  _rate_bono_views := COALESCE(_custom.custom_bono_visualizaciones_monto, _global_config.bono_visualizaciones_monto);
  _rate_bono_interactions := COALESCE(_custom.custom_bono_interacciones_monto, _global_config.bono_interacciones_monto);
  _rate_bono_votacion := COALESCE(_custom.custom_bono_votacion_monto, _global_config.bono_votacion_monto);
  _umbral_streams := _global_config.bono_streams_umbral;
  _umbral_views := _global_config.bono_visualizaciones_umbral;
  _umbral_interactions := _global_config.bono_interacciones_umbral;
  _umbral_votacion := _global_config.bono_votacion_umbral;

  -- VALID LEAD: present near start AND near end
  -- joined_at <= started_at + 2min  (entered from the beginning)
  -- (left_at IS NULL OR left_at >= ended_at - 2min)  (stayed until the end)
  SELECT count(DISTINCT COALESCE(vp.stream_lead_id::text, vp.correo)) INTO _valid_leads
  FROM public.live_viewer_presence vp
  WHERE vp.room_id = NEW.id
    AND (vp.stream_lead_id IS NOT NULL OR vp.correo IS NOT NULL)
    AND vp.joined_at <= (_started_at + _tolerance)
    AND (vp.left_at IS NULL OR vp.left_at >= (_ended_at - _tolerance));

  SELECT count(DISTINCT id) INTO _stream_viewers
  FROM public.live_viewer_presence WHERE room_id = NEW.id;

  SELECT count(*) INTO _stream_interactions
  FROM public.live_reactions WHERE room_id = NEW.id;

  SELECT COALESCE(avg(rating), 0) INTO _stream_avg_rating
  FROM public.live_stream_votes WHERE room_id = NEW.id;

  _period_start := date_trunc('month', _ended_at)::date;
  _period_end := (date_trunc('month', _ended_at) + interval '1 month' - interval '1 day')::date;

  -- Aggregate monthly using the same "full stream" rule for leads
  SELECT
    count(DISTINCT lr.id),
    COALESCE(sum(sub.viewers), 0) + _stream_viewers,
    COALESCE(sum(sub.leads), 0) + _valid_leads,
    COALESCE(sum(sub.interactions), 0) + _stream_interactions
  INTO _month_streams, _month_viewers, _month_leads, _month_interactions
  FROM public.live_rooms lr
  LEFT JOIN LATERAL (
    SELECT
      count(DISTINCT vp.id) as viewers,
      count(DISTINCT COALESCE(vp.stream_lead_id::text, vp.correo)) FILTER (
        WHERE (vp.stream_lead_id IS NOT NULL OR vp.correo IS NOT NULL)
          AND vp.joined_at <= (COALESCE(lr.started_at, lr.created_at) + _tolerance)
          AND (vp.left_at IS NULL OR vp.left_at >= (lr.ended_at - _tolerance))
      ) as leads,
      (SELECT count(*) FROM public.live_reactions r WHERE r.room_id = lr.id) as interactions
    FROM public.live_viewer_presence vp
    WHERE vp.room_id = lr.id
  ) sub ON true
  WHERE lr.host_id = NEW.host_id
    AND lr.status = 'ended'
    AND lr.id <> NEW.id
    AND lr.ended_at >= _period_start::timestamptz
    AND lr.ended_at < (_period_end + 1)::timestamptz;

  _month_streams := _month_streams + 1;

  SELECT COALESCE(avg(v.rating), 0) INTO _month_avg_rating
  FROM public.live_stream_votes v
  JOIN public.live_rooms lr ON lr.id = v.room_id
  WHERE lr.host_id = NEW.host_id
    AND lr.status = 'ended'
    AND lr.ended_at >= _period_start::timestamptz
    AND lr.ended_at < (_period_end + 1)::timestamptz;

  _earnings_leads := _month_leads * _rate_lead;
  _bonus_details := '{}'::jsonb;
  _earnings_bonuses := 0;

  IF _month_streams >= _umbral_streams AND _umbral_streams > 0 THEN
    _earnings_bonuses := _earnings_bonuses + _rate_bono_streams;
    _bonus_details := _bonus_details || jsonb_build_object('streams', _rate_bono_streams);
  END IF;
  IF _month_viewers >= _umbral_views AND _umbral_views > 0 THEN
    _earnings_bonuses := _earnings_bonuses + _rate_bono_views;
    _bonus_details := _bonus_details || jsonb_build_object('visualizaciones', _rate_bono_views);
  END IF;
  IF _month_interactions >= _umbral_interactions AND _umbral_interactions > 0 THEN
    _earnings_bonuses := _earnings_bonuses + _rate_bono_interactions;
    _bonus_details := _bonus_details || jsonb_build_object('interacciones', _rate_bono_interactions);
  END IF;
  IF _month_avg_rating >= _umbral_votacion AND _umbral_votacion > 0 THEN
    _earnings_bonuses := _earnings_bonuses + _rate_bono_votacion;
    _bonus_details := _bonus_details || jsonb_build_object('votacion', _rate_bono_votacion);
  END IF;

  INSERT INTO public.live_streamer_earnings (
    host_id, period_start, period_end,
    total_streams, total_viewers, total_leads, total_interactions, avg_rating,
    earnings_leads, earnings_bonuses, earnings_total,
    bonus_details, status
  ) VALUES (
    NEW.host_id, _period_start, _period_end,
    _month_streams, _month_viewers, _month_leads, _month_interactions, _month_avg_rating,
    _earnings_leads, _earnings_bonuses, _earnings_leads + _earnings_bonuses,
    _bonus_details, 'pending'
  )
  ON CONFLICT (host_id, period_start) DO UPDATE SET
    total_streams = EXCLUDED.total_streams,
    total_viewers = EXCLUDED.total_viewers,
    total_leads = EXCLUDED.total_leads,
    total_interactions = EXCLUDED.total_interactions,
    avg_rating = EXCLUDED.avg_rating,
    earnings_leads = EXCLUDED.earnings_leads,
    earnings_bonuses = EXCLUDED.earnings_bonuses,
    earnings_total = EXCLUDED.earnings_total,
    bonus_details = EXCLUDED.bonus_details,
    updated_at = now();

  RETURN NEW;
END;
$function$;

-- 2) Trigger to keep max_viewers / peak_viewers updated on every new viewer
CREATE OR REPLACE FUNCTION public.update_room_max_viewers()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
DECLARE
  _total int;
BEGIN
  SELECT count(*) INTO _total
  FROM public.live_viewer_presence
  WHERE room_id = NEW.room_id;

  UPDATE public.live_rooms
  SET max_viewers = GREATEST(COALESCE(max_viewers, 0), _total),
      peak_viewers = GREATEST(COALESCE(peak_viewers, 0), _total),
      updated_at = now()
  WHERE id = NEW.room_id;

  RETURN NEW;
END;
$function$;

DROP TRIGGER IF EXISTS trg_update_room_max_viewers ON public.live_viewer_presence;
CREATE TRIGGER trg_update_room_max_viewers
AFTER INSERT ON public.live_viewer_presence
FOR EACH ROW EXECUTE FUNCTION public.update_room_max_viewers();

-- 3) Backfill historical max_viewers / peak_viewers
UPDATE public.live_rooms lr
SET max_viewers = sub.cnt,
    peak_viewers = GREATEST(COALESCE(lr.peak_viewers, 0), sub.cnt)
FROM (
  SELECT room_id, count(*) AS cnt
  FROM public.live_viewer_presence
  GROUP BY room_id
) sub
WHERE lr.id = sub.room_id
  AND COALESCE(lr.max_viewers, 0) < sub.cnt;

-- 4) Recompute historical earnings using the new rule
WITH tol AS (SELECT interval '2 minutes' AS t),
ib_ext AS (
  SELECT user_id FROM public.user_roles WHERE role = 'ib_externo'
),
mon_ok AS (
  -- Hosts whose monetization is enabled (or not configured = enabled by default)
  SELECT ie.user_id AS host_id
  FROM ib_ext ie
  LEFT JOIN public.live_streamer_monetization lsm ON lsm.host_id = ie.user_id
  WHERE COALESCE(lsm.enabled, true) = true
),
config AS (
  SELECT * FROM public.live_monetization_config WHERE active = true LIMIT 1
),
per_room AS (
  SELECT
    lr.id AS room_id,
    lr.host_id,
    date_trunc('month', lr.ended_at)::date AS period_start,
    (date_trunc('month', lr.ended_at) + interval '1 month' - interval '1 day')::date AS period_end,
    (SELECT count(DISTINCT id) FROM public.live_viewer_presence vp WHERE vp.room_id = lr.id) AS viewers,
    (SELECT count(DISTINCT COALESCE(vp.stream_lead_id::text, vp.correo))
       FROM public.live_viewer_presence vp, tol
      WHERE vp.room_id = lr.id
        AND (vp.stream_lead_id IS NOT NULL OR vp.correo IS NOT NULL)
        AND vp.joined_at <= (COALESCE(lr.started_at, lr.created_at) + tol.t)
        AND (vp.left_at IS NULL OR vp.left_at >= (lr.ended_at - tol.t))
    ) AS leads,
    (SELECT count(*) FROM public.live_reactions r WHERE r.room_id = lr.id) AS interactions,
    (SELECT COALESCE(avg(rating), 0) FROM public.live_stream_votes v WHERE v.room_id = lr.id) AS avg_rating
  FROM public.live_rooms lr
  WHERE lr.status = 'ended'
    AND lr.host_id IN (SELECT host_id FROM mon_ok)
    AND lr.ended_at IS NOT NULL
),
agg AS (
  SELECT
    host_id, period_start, period_end,
    count(*)::int AS total_streams,
    sum(viewers)::int AS total_viewers,
    sum(leads)::int AS total_leads,
    sum(interactions)::int AS total_interactions,
    -- Weighted avg rating per room (simple avg of avg)
    COALESCE(avg(NULLIF(avg_rating, 0)), 0) AS avg_rating
  FROM per_room
  GROUP BY host_id, period_start, period_end
),
ranked AS (
  SELECT
    a.*,
    (SELECT dolares_por_lead FROM config) AS rate_lead,
    (SELECT bono_streams_umbral FROM config) AS u_streams,
    (SELECT bono_streams_monto FROM config) AS m_streams,
    (SELECT bono_visualizaciones_umbral FROM config) AS u_views,
    (SELECT bono_visualizaciones_monto FROM config) AS m_views,
    (SELECT bono_interacciones_umbral FROM config) AS u_int,
    (SELECT bono_interacciones_monto FROM config) AS m_int,
    (SELECT bono_votacion_umbral FROM config) AS u_vote,
    (SELECT bono_votacion_monto FROM config) AS m_vote
  FROM agg a
),
final AS (
  SELECT
    host_id, period_start, period_end,
    total_streams, total_viewers, total_leads, total_interactions, avg_rating,
    (total_leads * rate_lead) AS earnings_leads,
    (CASE WHEN u_streams > 0 AND total_streams >= u_streams THEN m_streams ELSE 0 END
     + CASE WHEN u_views > 0 AND total_viewers >= u_views THEN m_views ELSE 0 END
     + CASE WHEN u_int > 0 AND total_interactions >= u_int THEN m_int ELSE 0 END
     + CASE WHEN u_vote > 0 AND avg_rating >= u_vote THEN m_vote ELSE 0 END) AS earnings_bonuses
  FROM ranked
)
INSERT INTO public.live_streamer_earnings AS lse (
  host_id, period_start, period_end,
  total_streams, total_viewers, total_leads, total_interactions, avg_rating,
  earnings_leads, earnings_bonuses, earnings_total, bonus_details, status
)
SELECT
  host_id, period_start, period_end,
  total_streams, total_viewers, total_leads, total_interactions, avg_rating,
  earnings_leads, earnings_bonuses, earnings_leads + earnings_bonuses,
  '{}'::jsonb, 'pending'
FROM final
ON CONFLICT (host_id, period_start) DO UPDATE SET
  period_end = EXCLUDED.period_end,
  total_streams = EXCLUDED.total_streams,
  total_viewers = EXCLUDED.total_viewers,
  total_leads = EXCLUDED.total_leads,
  total_interactions = EXCLUDED.total_interactions,
  avg_rating = EXCLUDED.avg_rating,
  earnings_leads = EXCLUDED.earnings_leads,
  earnings_bonuses = EXCLUDED.earnings_bonuses,
  earnings_total = EXCLUDED.earnings_total,
  -- Preserve "paid" status; otherwise reset to pending
  status = CASE WHEN lse.status = 'paid' THEN 'paid' ELSE 'pending' END,
  updated_at = now();
