...
-- ----------------------------------------------------------------------------
-- 1) Sincroniza cada CURSO de pago con su fila vendible en portal_products
--    (product_type='course', reference_id=course_id), igual que ya hacen los
--    bundles. Así un curso puede venderse individualmente y tener override.
-- 2) Reescribe distribute_order_commissions para repartir la RED por LÍNEA de la
--    orden: por cada item usa los niveles propios del producto
--    (portal_product_commission_levels) si existen, si no los generales
--    (portal_mlm_levels). Cap por línea = lineTotal × pool%. Órdenes sin items
--    (eventos/planes) usan el total × niveles generales (como hasta ahora).
--    El resto (fee, socios, IB residual, compresión, host-defer) SIN CAMBIOS.
-- Idempotente. No destruye datos.
-- ============================================================================

-- ─────────────────────────────────────────────────────────────────────────
-- 1) SYNC: curso de pago → portal_products vendible
-- ─────────────────────────────────────────────────────────────────────────
...
DECLARE _pid uuid;
BEGIN
  -- Canónica = la más antigua (determinista). FK-safe: nunca se borra (order_items
  -- puede referenciarla); las duplicadas legadas se DESACTIVAN.
  SELECT id INTO _pid
  ...
  IF COALESCE(NEW.is_free, false) = false AND COALESCE(NEW.price_usd, 0) > 0 THEN
    ...
    ELSE
      UPDATE public.portal_products
       ...
       WHERE id = _pid;
    END IF;
    -- Dedup: desactivar cualquier otra fila vendible del mismo curso.
    UPDATE public.portal_products SET status = 'inactive', updated_at = now()
     WHERE portal_id = NEW.portal_id AND product_type = 'course' AND reference_id = NEW.id
       AND id <> _pid AND status <> 'inactive';
  ELSE
    -- Gratis / sin precio → desactivar TODAS las vendibles del curso.
    UPDATE public.portal_products SET status = 'inactive', updated_at = now()
     WHERE portal_id = NEW.portal_id AND product_type = 'course' AND reference_id = NEW.id
       AND status <> 'inactive';
  END IF;
...
-- Backfill de cursos existentes de pago.
DO $$
...
-- ─────────────────────────────────────────────────────────────────────────
-- 2) MOTOR por línea (override por producto)
-- ─────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF _host IS NULL THEN RETURN; END IF;  -- difiere a reconciliación

  -- (a) Platform fee (Bullfy), debitado del 100%.
  ...
  -- (b) Pool de red con COMPRESIÓN, POR LÍNEA (override por producto) o por total.
  ...
  IF COALESCE(_mlm_enabled,false) AND COALESCE(_pool_pct,0) > 0 THEN
    ...
    IF _eligible IS NOT NULL AND array_length(_eligible,1) > 0 THEN
      IF EXISTS (SELECT 1 FROM public.portal_order_items WHERE order_id = _order_id) THEN
        ...
        END LOOP;
      ELSE
        -- ── SIN line items (eventos/planes): total × niveles generales ──
  ...
  -- (c) Socios (incluye al IB): reparto del pot. Otros socios pro-rata; IB residual.
